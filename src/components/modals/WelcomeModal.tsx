...
import { useAppContext } from '../../context/AppContext'; // 1. ADDED THIS

// REMOVED: import { SCHOOL_CREST_URL } from '../../constants';
...
const WelcomeModal: React.FC<WelcomeModalProps> = ({ isOpen, onClose }) => {
    const { state } = useAppContext(); // 2. GET a
    const { crestUrl } = state.settings; //    the global state

    return (
        <Modal isOpen={isOpen} onClose={onClose} ariaLabel="Welcome">
            <div className="bg-slate-50 dark:bg-slate-800 rounded-lg shadow-xl p-8 w-full max-w-3xl text-center">
                {/* 3. USE the correct crestUrl from state */}
