...
const ExamHeader: React.FC = () => {
    ...
    // Choose the grid layout based on whether the crest is enabled.
    // This prevents a blank column from appearing.
    const gridLayoutClass = settings.showCrest
        ? 'grid-cols-[auto,1.5fr,2fr,1.5fr]' // 4 columns when crest is ON
        : 'grid-cols-[1.5fr,2fr,1.5fr]';      // 3 columns when crest is OFF
    ...
    return (
        <header className="p-4 border-b-4 border-slate-200 dark:border-slate-700">
            <div className={`grid ${gridLayoutClass} items-center gap-x-6 w-full`}>
                ...
                {/* --- Column 1: Crest (Conditionally Rendered) --- */}
                {/* This entire column is removed from the DOM if showCrest is false */}
                {settings.showCrest && (
