...
const ActionButton: React.FC<{
    ...
    className?: string,
    disabled?: boolean
}> = ({ onClick, tooltipText, children, className = '', disabled = false }) => {
    const showTooltips = useStore(state => state.ui.showTooltips);
    const { showTooltip, hideTooltip } = useTooltip();
    ...
    return (
       <button 
            ...
            onMouseOver={handleMouseOver}
            onMouseOut={hideTooltip}
            className={`flex items-center gap-2 px-6 py-3 font-semibold rounded-full shadow-lg transition disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
        >
            {children}
        </button>
    );
...
const ExamActions: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-16 right-4 flex flex-col md:flex-row items-end gap-2 z-50">
            <div className={`flex flex-wrap justify-end items-center gap-2 transition-all duration-300 ${ui.fabsCollapsed ? 'hidden' : ''}`}>
                {/* Controls */}
                ...
                <div className="flex flex-wrap justify-end items-center gap-2">
                    {!isLive ? (
                        <>
                             <ActionButton disabled={exams.length === 0} onClick={() => dispatch({type: 'SET_ACTIVE_MODAL', payload: 'autoStart'})} tooltipText="Set a specific time for the exams to commence automatically." className="bg-slate-600 text-white hover:bg-slate-700">
                             ...
                             <ActionButton onClick={() => dispatch({ type: 'END_SESSION', payload: {shouldReset: false} })} tooltipText="Go back to the setup page to add or remove exams." className="bg-slate-600 text-white hover:bg-slate-700">
                                ...
                                <span className="hidden sm:inline">Return to Setup</span>
                             </ActionButton>
                             <ActionButton disabled={exams.length === 0} onClick={handleCommence} tooltipText="Start all exam timers. This will begin the session." className="bg-green-600 text-white hover:bg-green-700">
                        ...
                        <>
                        {isSessionPaused ? (
                            <ActionButton onClick={() => dispatch({ type: 'RESUME_SESSION', payload: {} })} tooltipText="Resume the paused session." className="bg-green-600 text-white hover:bg-green-500 animate-pulse">
                        ...
                        {isSessionPaused ? (
                            ...
                            <ActionButton onClick={() => dispatch({ type: 'SET_CONFIRM_ACTION', payload: {type: 'endAndReset'} })} tooltipText="End the session and clear all exams and settings." className="bg-red-600 text-white hover:bg-red-700">
                                ...
                                <span className="hidden sm:inline">End & Reset</span>
                            </ActionButton>
                        </>
                    )}
                </div>
